...
public class TestCreateBooking extends BaseTest {
    ...
    @Test(groups = "reg", priority = 1)
    @Owner("Sai")
    @Description("Tc#1 - Verify that the booking be created when payload is wrong")
    public void testCreateBookingPost_NEGATIVE() {

        //setup and making a request.
        requestSpecification.basePath(APIConstants.CREATE_UPDATE_BOOKING_URL);
        response = RestAssured.given(requestSpecification).when().body("{}").log(payloadManager);
    ...
    public void testCreateBookingPost_POSITIVE_RANDOM_DATA() {
        ...
        //setup and making a request.
        requestSpecification.basePath(APIConstants.CREATE_UPDATE_BOOKING_URL);
        response = RestAssured.given(requestSpecification).when().body(payloadManager).log();
        System.out.println(response.asString());
